...
export default function PaymentAgingForeign() {
  const { paymentAgingForeign, setPaymentAgingForeign } = useStore()
  const [loading, setLoading] = useState(!paymentAgingForeign)
  ...
  const { order: colOrder, moveColumn } = useColumnOrder('payment-aging-foreign', ALL_COLS)
  const [dragCol, setDragCol] = useState(null)
  const [dragOverCol, setDragOverCol] = useState(null)

  useEffect(() => {
    if (paymentAgingForeign) {
      setLoading(false)
      return
    }
    setLoading(true)
  ...
  }, [paymentAgingForeign, setPaymentAgingForeign])
  ...
  const rows = paymentAgingForeign?.rows || []
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] space-y-4">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">
          Processing Foreign Vendor Payment Aging analysis...
        </p>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        ...
        <span className="hidden md:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-slate-100 dark:bg-slate-800 text-[11px] font-semibold text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700">
          ...
          Foreign Vendors (Non-INR)
        </span>
      </div>

      {/* AI Insight Box */}
      <AiInsightBox section="paymentagingforeign" kpis={{
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm">
          <h3 className="text-sm font-bold text-slate-900 dark:text-white mb-3">Top 5 Overdue Foreign Vendors</h3>
          <div className="overflow-x-auto max-h-[350px] overflow-y-auto">
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800 font-medium">
                ...
                {topOverdue.length === 0 && (
                  <tr>
                    <td colSpan={5} className="py-4 text-center text-slate-400">No overdue vendors found</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-800 flex flex-col gap-3">
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">Detail level aging status for each foreign vendor ledger transaction</p>
            </div>
