...
def run_payment_aging_msme(dfs: dict[str, pd.DataFrame]) -> dict:
    """
    Computes Payment Aging (MSME Compliance) analysis.
    Filters to only include vendors with MSME Registration value of 'micro', 'small', or 'Y' (case-insensitive).
    Adds 'Company Type' column after Vendor Address.
    ...
    # 1. Load BP Master for payment terms and MSME registration
    bp_terms = {}
    bp_msme = {}
    if df_bp is not None and not df_bp.empty:
        ...
        col_msme = None
        for alias in [
            "msme registration", "msme_registration", "msme registeration", "msme",
            "msme category", "msme_category", "msme reg", "msme type",
            "enterprise type", "msmereg", "udyam category", "udyam type", "msme status"
        ]:
            for col in df_bp.columns:
                if str(col).lower().strip() == alias:
                    col_msme = col
        ...
        if col_bp_code:
            cols_to_use = [col_bp_code]
            if col_pt_code:
                cols_to_use.append(col_pt_code)
            if col_msme:
                cols_to_use.append(col_msme)
            ...
            for row in bp_records:
                ...
                if not code:
                    continue
                if col_pt_code:
                    bp_terms[code] = str(row.get(col_pt_code, '')).strip()
                if col_msme:
                    bp_msme[code] = str(row.get(col_msme, '')).strip()
    ...
    if df_gl is not None and not df_gl.empty:
        ...
        if has_vendor_headers:
            ...
            for row in gl_rows:
                ...
                msme_val = bp_msme.get(current_vendor_code, "")
                if not isinstance(msme_val, str):
                    msme_val = ""
                msme_val_clean = msme_val.strip()
                msme_val_lower = msme_val_clean.lower()
                if msme_val_lower not in ("micro", "small", "y"):
                    continue
        ...
        for vcode, txs in vendor_transactions.items():
            ...
            vendor_group = v_info.get('group', 'MSME vendor')
            if not vendor_group or vendor_group.lower() in ('nan', 'none', ''):
                vendor_group = 'MSME vendor'
                
            vendor_address = v_name
            company_type = bp_msme.get(vcode, "—")
