...
def run_payment_aging_foreign(dfs: dict[str, pd.DataFrame]) -> dict:
    """
    Computes Payment Aging (Foreign) analysis.
    Filters out any vendor where Vendor Country is "India" (derived from Document Currency == "INR").
        ...
        if has_vendor_headers:
            ...
            if col_bp_idx is not None:
                for row in gl_rows:
                    ...
                    v_info = po_vendor_info.get(c_str, {})
                    country = v_info.get('country', 'India')
                    if country == 'India':
                        continue
