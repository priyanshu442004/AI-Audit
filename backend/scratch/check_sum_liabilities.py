...
for vcode, txs in vendor_transactions.items():
    ...
    if final_bal < 0:
        sum_liabilities += abs(final_bal)

print(f"Total domestic vendors in transactions: {len(vendor_transactions)}")
print(f"Sum of negative final balances: {sum_liabilities:.2f} INR = {sum_liabilities/1e7:.2f} Cr")
