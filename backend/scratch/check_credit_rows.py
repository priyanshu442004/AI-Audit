...
for row in gl_rows:
    ...
    total_rows_count += 1
    credit = float(str(row[12]).replace(",", "").strip()) if (len(row) > 12 and row[12] is not None and str(row[12]).strip() != "") else 0.0
    if credit > 0:
        credit_rows_count += 1

print(f"Total domestic vendor rows: {total_rows_count}")
print(f"Total domestic credit rows: {credit_rows_count}")
