...
for vcode, txs in vendor_transactions.items():
    # Final cumulative balance of the vendor is in the last row's cumulative balance column (col 13)
    last_tx = txs[-1]
    ...
    # If final_bal >= 0, they have an advance or zero balance (no AP liability)
    if final_bal >= 0:
        continue
        
    liability = abs(final_bal)
    
    # Now walk backwards to find invoices (Credit > 0)
    allocated = 0.0
...
print(f"Backward-matching total domestic outstanding: {total_outstanding:.2f} INR = {total_outstanding/1e7:.2f} Cr")
